...
(define (modexpmod base exp n)
  (define (square-mod expresult)
    (define (non-trivial-root? x)
      (if (or (= x 1) (= x (- n 1)))
          #f
          (= (remainder (square x) n) 1)))
    (if (non-trivial-root? expresult) 0 (square expresult)))
  (cond ((= exp 0) 1)
        ((even? exp)
         (remainder (square-mod (modexpmod base (/ exp 2) n))
                    n))
        (else
          (remainder (* base (modexpmod base (- exp 1) n))
                     n))))
...
(define (mill-rab n)
  (define (try-it a)
    (define (check x)
      (not (or (= x 0) (> x 1))))
    (check (modexpmod a (- n 1) n)))
  (try-it (+ 1 (pseudo-random-integer (- n 1)))))

(define (fast-prime-mr? n times)
  (cond ((= times 0) #t)
        ((mill-rab n) (fast-prime-mr? n (- times 1)))
        (else #f)))

(define (prime-mr? n)
  (fast-prime-mr? n 100))

; And it works for the Carmichael numbers
(prime-mr? 561)
(prime-mr? 1105)
(prime-mr? 1729)
(prime-mr? 2465)
